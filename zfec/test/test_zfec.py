...
def _h(k, m, ss):
    ...
    assert tuple([str(s) for s in decoded]) == tuple([str(s) for s in ss]), (tuple(
        [ab(str(s)) for s in decoded]), tuple([ab(str(s)) for s in ss]),)
...
class ZFecTest(unittest.TestCase):
    def test_instantiate_encoder_no_args(self):
        try:
            ...
            self.fail(
                "Should have raised exception from incorrect arguments to constructor.")
    ...
    def test_small(self):
        ...
        if VERBOSE:
            print("{} randomized tests pass.".format(i+1))
...
class FileFec(unittest.TestCase):
    ...
    def _help_test_filefec(self, teststr, k, m, numshs=None):
        ...
        try:
            ...
            # encode the file
            zfec.filefec.encode_to_files(
                tempf, fsize, tempdir.name, PREFIX, k, m, SUFFIX, verbose=VERBOSE)
            ...
            sharefs = [open(os.path.join(tempdir.name, fn), "rb")
                       for fn in fns if RE.match(fn)]
...
class Cmdline(unittest.TestCase):
    def test_basic(self, noisy=VERBOSE):
        ...
        try:
            ...
            fns = os.listdir(tempdir.name)
            assert len(fns) >= DEFAULT_M, (fns,
                                           DEFAULT_M, tempdir, tempdir.name,)
            sharefns = [os.path.join(tempdir.name, fn)
                        for fn in fns if RE.match(fn)]
            ...
            sys.argv.extend(
                ['-o', os.path.join(tempdir.name, 'test.data-recovered'), ])
